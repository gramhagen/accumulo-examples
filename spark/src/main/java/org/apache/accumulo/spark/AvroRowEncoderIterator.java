...
public class AvroRowEncoderIterator extends BaseMappingIterator {
	...
	@Override
	protected void processCell(Key key, Value value, String column, Object decodedValue) throws IOException {
	...
	public void init(SortedKeyValueIterator<Key, Value> source, Map<String, String> options, IteratorEnvironment env)
		...
		for (Map.Entry<String, SchemaMappingField> entry : schemaMapping.getMapping().entrySet()) {
			...
			fields.add(new Schema.Field(entry.getKey(), Schema.create(getAvroType(field.getType())), null, null));
		}
